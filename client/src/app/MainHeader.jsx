...
export default function MainHeader({ channel, isHome, membersVisible, onToggleMembers, onToast, onOpenDrawer }) {
  const title = isHome ? 'Freunde' : channel?.name || 'Channel';
  return (
    <header className="main-header">
      ...
      <div className="main-header__title">
        {isHome ? <Users size={22} /> : <Hash size={22} />}
        <strong>{title}</strong>
      ...
      <div className="main-header__actions">
        ...
        <button className="icon-button" type="button" aria-label="Benachrichtigungen" onClick={() => onToast('Benachrichtigungen folgen später.')}><Bell size={20} /></button>
        ...
        {!isHome && <button className={`icon-button ${membersVisible ? 'is-active' : ''}`} type="button" aria-label="Mitgliederliste umschalten" onClick={onToggleMembers}><Users size={21} /></button>}
        <button className="icon-button header-search" type="button" aria-label="Suche" onClick={() => onToast('Die Nachrichtensuche folgt in Phase 3.')}><span>Suche</span><Search size={17} /></button>
