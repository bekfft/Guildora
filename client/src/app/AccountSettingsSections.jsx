...
export function PrivacySection({ settings, save, onToast }) {
  ...
  const reloadSafety = () => api.accountSafety().then(setSafety);
  useEffect(() => { reloadSafety().catch((e) => onToast(e.message, 'error')); }, []);
...
function UserAppealThread({ value, onClose, onToast, onRefresh }) {
  ...
  return <div className="user-appeal-thread"><div className="user-appeal-thread__header"><div><strong>Einspruchsverlauf</strong><small>Status: {value.appeal.status}</small></div><button type="button" onClick={onClose} aria-label="Einspruchsverlauf schließen">×</button></div><div className="user-appeal-messages">{value.messages.map((message)=><article key={message.id} className={message.is_staff?'is-staff':''}><div><strong>{message.is_staff?'Guildora Staff':'Du'}</strong><time>{new Date(message.created_at).toLocaleString('de-DE')}</time></div><p>{message.body}</p></article>)}</div>{closed?<p className="user-appeal-closed">Dieser Einspruch wurde abgeschlossen.</p>:<form onSubmit={async(event)=>{event.preventDefault();if(!body.trim())return;try{await api.addMyAppealMessage(value.appeal.id,body);setBody('');await onRefresh(value.appeal.id);}catch(error){onToast(error.message,'error');}}}><textarea value={body} onChange={(event)=>setBody(event.target.value)} placeholder="Rückfrage oder Ergänzung an das Staff-Team …"/><Button type="submit">Nachricht senden</Button></form>}</div>;
}
